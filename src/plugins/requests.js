...
export const getUserCityByIP = async () => {
  let response = await axios.get("https://ipinfo.io");
  let city = response.data.city;

  let countriesInfo = await getCountry(city);
  ...
  if (countriesInfo.length == 0) {
    ...
    let countryInfo = countriesInfo[1];
    resultObj.city = countryInfo.city;
    resultObj.country = countryInfo.country;
    resultObj.code = countryInfo.code;
