        ...
        <div class="content__search">
            ...
            <div class="container-menu" v-if="tickets.length > 0">
                <div class="menu-sort" @click="sort = 1">
                    ...
                    <div class="sort-discript">from {{ tickets[0].price }} {{ currency }}</div>
        ...
        <div class="container__list_ticket" v-if="tickets.length > 0">
            <div class="content__list_ticket">
                <div class="list__tickets">
                    <ticket-vue v-for="(ticket, index) in tickets" :key="index" :price="ticket.price">
                        <template v-slot:airline_1>{{ ticket.airline }}</template>
                        <template v-slot:airline_2></template>
                        <template v-slot:depart_time>{{ ticket.departureTime.split(" ")[1] }}</template>
                        <template v-slot:arrival_time>{{ ticket.arrivalTime.split(" ")[1] }}</template>
                        <template v-slot:depart_city
                            >{{ ticket.departureTime.split(" ")[0] }}, {{ ticket.departureAirport }}</template
                        >
                        <template v-slot:arrival_city
                            >{{ ticket.arrivalTime.split(" ")[0] }}, {{ ticket.arrivalAirport }}</template
                        >
                                ...
                                @click="
                                    this.$router.push({
                                        ...
                                        params: {
                                            ...
                                            depart_city: ticket.departureTime.split(' ')[0] + ' ' + ticket.departureAirport,
                                            arrival_city: ticket.arrivalTime.split(' ')[0] + ' ' + ticket.arrivalAirport,
